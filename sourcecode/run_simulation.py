...
if __name__ == "__main__":
    ...
    # simulaterディレクトリのパスを取得
    simulater_dir_path = os.path.abspath("../")
    ...
    if not os.path.exists(f"{algorithm}/"):
        ...
        config_paths = [os.path.join(config_dir_path, file_name)
        ...
        command_create_dags = "python3 run_generator.py -c {config} -d {simulater_dir_path}/DAGs/Max_utilization-{utilization}"
        for i, config in enumerate(config_paths):
            full_command = command_create_dags.format(
                config=config,
                simulater_dir_path=simulater_dir_path,
                utilization=max_utilization[i])
            print(full_command)
            os.system(full_command)
        print("----------Finish----------")

        os.chdir(simulater_dir_path + "/sourcecode")
        ...
        command_devide_files = "python3 devide_files.py -s ../DAGs/{DAGs_dir}/DAGs/ -n {folder_num} -o {algorithm}/UsedDag/{DAGs_dir}/"
        for DAGs_dir in DAGs_dirs:
            full_command = command_devide_files.format(DAGs_dir=DAGs_dir,
        ...
        for config in config_paths:
            full_command = command.format(config=config,
                                          algorithm=algorithm,
                                          utilization=max_utilization[i])
            print(full_command)
            os.system(full_command)
    ...
    # all_execute.pyを実行
    print("----------All execute----------")
    execute_simulater_path = os.path.abspath(args["simulator"])
    core_num = args["core_num"]
    command_all_execute = f"python3 all_execute.py -e {execute_simulater_path} -d {algorithm}/UsedDag/ -c {core_num}"
